...
import {
  ...
  Switch, HStack, VStack, useToast, Text, Divider, Spinner,
...
export default function EditNewsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    axios.get(`/api/articles/${id}`)
      ...
      .catch(() => {
        toast({ title: 'Мэдээ олдсонгүй', status: 'error', duration: 3000 });
        router.push('/news');
      })
      .finally(() => {
        setFetching(false);
  ...
  const handleSubmit = async (published: boolean) => {
    ...
    setLoading(true);
    try {
      await axios.put(`/api/articles/${id}`, { ...form, published });
  ...
  if (fetching) {
    return (
      <Box px={6} py={12} textAlign="center">
        ...
        <Text mt={2} color="gray.500">Уншиж байна...</Text>
      </Box>
    );
  }
  ...
  return (
    <Box px={6} py={6} maxW="900px">
      <Breadcrumb items={[{ label: 'Мэдээ', href: '/news' }, { label: 'Засах' }]} />

      <HStack justify="space-between" mb={6}>
        <Heading size="lg" color="gray.800">Мэдээ засах</Heading>
        <LinkNext href="/news">
